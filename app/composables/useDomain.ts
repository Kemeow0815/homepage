...
export function getDomainType(domain: string): string {
	if (domain.endsWith('.cn')) return '中国大陆域名'
	if (domain.endsWith('.com')) return '国际通用域名'
	if (domain.endsWith('.org')) return '非营利组织域名'
	if (domain.endsWith('.net')) return '网络服务机构域名'
	if (domain.endsWith('.io')) return '科技/初创公司域名'
	if (domain.endsWith('.dev')) return '开发者域名'
	if (domain.endsWith('.me')) return '个人域名'
	if (domain.endsWith('.blog')) return '博客域名'
	return '其他域名'
}
...
export function getDomainIcon(url: string): string | null {
	...
	if (domain.includes('bilibili')) return 'ri:bilibili-fill'
	if (domain.includes('zhihu')) return 'ri:zhihu-line'
