...
function handlePointerMove(e: PointerEvent) {
	if (!bgImage.value || !activePointers.value.has(e.pointerId))
		return
	e.preventDefault()
	activePointers.value.set(e.pointerId, { x: e.clientX, y: e.clientY })
...
async function doExport() {
	...
	if (border)
		(border as SVGElement).style.display = 'none'
	...
	if (border)
		(border as SVGElement).style.display = ''
}
				...
				<div class="control-group">
					<div class="form-item">
						...
						<div class="upload-wrapper">
							<input id="bg-upload" type="file" accept="image/*" class="hidden" @change="handleBgImageUpload">
							<label
								for="bg-upload"
								class="upload-area"
					...
					<div class="form-item">
						...
						<div class="upload-wrapper">
							<input id="font-upload" type="file" accept=".ttf,.otf,.woff,.woff2" class="hidden" @change="handleFontUpload">
						...
						<p v-if="localFontError" class="error-text">
							{{ localFontError }}
						</p>
						<div v-if="localFonts.length > 0" class="font-list">
							...
							<button class="close-btn" @click="localFonts = []; localFontSearchQuery = ''">
								关闭
							</button>
						</div>
					</div>
					...
					<div class="form-item">
						...
						<div class="icon-grid" :class="{ disabled: !showIcon }">
							<div class="upload-wrapper">
								<input id="icon-upload" type="file" accept="image/*" class="hidden" @change="handleLocalIconUpload">
				...
				<div class="card">
					...
					<div class="card-body">
						...
						<div class="form-item compact">
							...
							<p v-if="activeRatios.length === 0" class="error-text">
								请至少选择一个画板比例以进行导出
							</p>
							<p v-else-if="activeRatios.length === 1" class="hint-text">
								当前仅预览 {{ activeRatios[0].label }}，将导出此尺寸
							</p>
							<p v-else class="hint-text right">
								不选默认导出预览选中比例
							</p>
						</div>
					</div>
